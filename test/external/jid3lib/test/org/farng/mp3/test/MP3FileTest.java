 ...
 */
public class MP3FileTest extends VanillaTest {
	...
	private ArrayList<MediaSource> mOpenedMediaSources = new ArrayList<MediaSource>();
...
//        }
     ...
     * @throws Exception
     */
    // dbeswick: saving not yet necessary in vanilla
    ...
    // dbeswick
    protected File getTestFile(String filePath) throws MediaAccessException {
    	MediaSource newSource = new MediaSourceAndroidAsset(getTestAssets(), filePath);
    	mOpenedMediaSources.add(newSource);
    	
   		return newSource.getFile();
    }
    
    protected File getTestFile(File file) throws MediaAccessException {
    	return getTestFile(file.getPath());
    }
    ...
    protected void setUp() throws Exception {
        File parentDir = getInstrumentation().getContext().getExternalCacheDir();
        if (parentDir == null)
        	throw(new Exception("Couldn't find the test directory."));

        // make a temp directory name
        this.testMusicDirectory = new File(parentDir, "test-working");
        this.testMusicOriginalDirectory = new File("jid3lib/music");
        ...
        String[] originalFileArray = getTestAssets().list(testMusicOriginalDirectory.getPath());
        ...
        for (int i = 0; i < originalFileArray.length; i++) {
        	File sourceFile = getTestFile(testMusicOriginalDirectory.getPath() + "/" + originalFileArray[i]);
            newFile = new File(this.testMusicDirectory, originalFileArray[i]);
            try {
                TagUtility.copyFile(sourceFile, newFile);
            } catch (Exception ex) {
                System.out.println("Error copying original music file: " + sourceFile.getPath());
                throw(ex);
            }
        }
    ...
    protected void tearDown() {
    	for (MediaSource mediaSource : mOpenedMediaSources) {
    		mediaSource.close();
    	}
